...
// Station strategy - Limited view, mainly for viewing other station users
export class UserStationStrategy extends UserManagementStrategy {
  transformTableData(users) {
    // Stations see limited data, mainly other station users
    return users
      .filter(user => user.role === 'Station' || user._id === this.userEntiteId)
      .map(user => ({
        ...user,
  ...
  getAvailableFilters() {
    return [
      'status' // Only status filter for stations
    ];
  }
  ...
  getTableColumns() {
    return [
      ...
      { key: 'entiteId', label: 'Station', sortable: false },
  ...
  getAvailableActions(user) {
    // Stations can only view, no actions on other users
    return user._id === this.userEntiteId ? ['edit'] : [];
  }

  canPerformAction(action, user) {
    // Stations can only edit their own profile
