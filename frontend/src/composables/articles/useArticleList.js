...
export function useArticleList() {
  ...
  const performFetch = async () => {
    ...
    // Fetch available categories from the complete filtered dataset (all pages)
    await updateAvailableCategories();
  };
  
  /**
   * Fetch and update available categories for the current filter set
   */
  const updateAvailableCategories = async () => {
    try {
      const filterValues = getFilterValues();
      // Remove category filter to get all categories in the filtered dataset
      const { categorie, ...filtersWithoutCategory } = filterValues;
      
      const availableCategories = await fetchAvailableCategories(filtersWithoutCategory);
      // Set dataset categories directly with the fetched categories
      setDatasetCategories(availableCategories);
    } catch (error) {
      console.warn('Failed to fetch available categories, keeping current ones');
    }
  };
