...
export function useFournisseurList() {
  ...
  /**
   * Handle fournisseur data updated with refresh (without closing panel)
   */
  const handleFournisseurDataUpdatedWithRefresh = async () => {
    handleFournisseurDataUpdated();
    await performFetch();
    
    // Update the selected fournisseur with fresh data
    if (selectedFournisseurForEdit.value) {
      const updatedFournisseur = fournisseurs.value.find(f => f._id === selectedFournisseurForEdit.value._id);
      if (updatedFournisseur) {
        selectedFournisseurForEdit.value = updatedFournisseur;
      }
    }
  };
