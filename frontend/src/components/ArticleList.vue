...
<template>
    <div class="panel">
        ...
        <div class="panel-body">
            <a-table :columns="columns" :data-source="articles" :loading="isLoading" row-key="_id">
    ...
    <a-drawer title="Ajouter un nouvel article" :width="500" :open="isCreateDrawerVisible" @close="closeCreateDrawer">
        <a-form :model="createFormState" layout="vertical">
            ...
            <a-form-item label="Catégorie" required>
                <a-select v-model:value="createFormState.categorie" placeholder="Sélectionnez une catégorie"
                    :options="categories">
                </a-select>
            </a-form-item>
        </a-form>
    ...
    <a-drawer :title="`Gérer les fournisseurs pour : ${editingArticle?.codeArticle}`" :width="600"
        :open="isManageDrawerVisible" @close="closeManageDrawer" :footer-style="{ textAlign: 'right' }">
        <h4>Fournisseurs associés</h4>
        <a-list :data-source="editingArticle?.fournisseurs" item-layout="horizontal">
            <template #renderItem="{ item }">
                <a-list-item>
                    <a-list-item-meta :title="item.fournisseurId?.nom || 'Fournisseur introuvable'"
                        :description="`Prix: ${item.prixUnitaire} € - Réf: ${item.referenceFournisseur || 'N/A'}`" />
                    <a-popconfirm title="Sûr de vouloir supprimer ce lien ?"
                        @confirm="handleRemoveSupplierLink(item._id)">
        ...
        <a-form :model="addSupplierForm" layout="vertical">
            <a-form-item label="Fournisseur">
                <a-select v-model:value="addSupplierForm.fournisseurId" placeholder="Sélectionner un fournisseur"
                    :options="availableSuppliers.map(f => ({ value: f._id, label: f.nom }))" show-search />
            </a-form-item>
            ...
            <a-form-item label="Prix Unitaire (€)">
                <a-input-number v-model:value="addSupplierForm.prixUnitaire" :min="0" style="width: 100%" />
            </a-form-item>
            <a-form-item>
                <a-button type="primary" @click="handleAddSupplierLink">
                    <PlusOutlined /> Ajouter ce fournisseur
                </a-button>
            </a-form-item>
        </a-form>
