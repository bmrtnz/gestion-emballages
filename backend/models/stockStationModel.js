...
// Schéma principal pour le stock station
...
// Index composé pour garantir l'unicité du stock par station/campagne
stockStationSchema.index({ stationId: 1, campagne: 1 }, { unique: true });

// Index pour améliorer les performances de recherche
stockStationSchema.index({ "weeklyStocks.numeroSemaine": 1 });
stockStationSchema.index({ "weeklyStocks.articles.articleId": 1 });

// Méthode pour obtenir ou créer une semaine
stockStationSchema.methods.getOrCreateWeek = function (numeroSemaine) {
...
stockStationSchema.methods.updateArticleWeeklyStock = function (numeroSemaine, articleId, quantiteStock) {
...
stockStationSchema.methods.updateWeeklyStocks = async function (numeroSemaine, articlesData) {
    try {
        const weekStock = this.getOrCreateWeek(numeroSemaine);
...
stockStationSchema.methods.getWeeklyStock = function (numeroSemaine) {
...
stockStationSchema.methods.getArticleStockForWeek = function (numeroSemaine, articleId) {
    const weekStock = this.weeklyStocks.find(w => w.numeroSemaine === numeroSemaine);
...
stockStationSchema.methods.getArticleHistory = function (articleId) {
    const history = [];
...
stockStationSchema.methods.getArticleTotalStock = function (articleId) {
    let total = 0;
...
stockStationSchema.methods.getQuarterlyStats = function () {
    const quarters = {
...
stockStationSchema.methods.getWeeksWithStock = function () {
...
stockStationSchema.methods.getAllArticles = function () {
